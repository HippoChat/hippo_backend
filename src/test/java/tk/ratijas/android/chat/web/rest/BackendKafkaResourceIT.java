...
import tk.ratijas.android.chat.config.KafkaProperties;
...
class BackendKafkaResourceIT {
    ...
    void setup() {
        ...
        BackendKafkaResource kafkaResource = new BackendKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/backend-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/backend-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
